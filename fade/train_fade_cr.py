...
def train(
    ...
    target_class = settings.target
    negative_prompts = ["A photo of a "+target_class,"A photo of many "+target_class,"A photo of more than two "+target_class]
    print("negative prompts ==> ",negative_prompts)
    with torch.no_grad():

        #NDL 
        negative_prompt_embeddings = []
        for i,np in enumerate(negative_prompts):
            negative_prompt_embeddings.append(train_util.encode_prompts(tokenizer,text_encoder,[np]))
        ...
        for settings in prompts:
            for prompt in [
                ...
                settings.retain_one,
                settings.retain_two,
                settings.retain_three,
                settings.retain_four,
                settings.retain_five
            ]:
            ...
            prompt_pair = PromptEmbedsPair(
                ...
                cache[settings.retain_one],
                cache[settings.retain_two],
                cache[settings.retain_three],
                cache[settings.retain_four],
                cache[settings.retain_five],
                settings,
            )

            print("prompt pair ::",prompt_pair)
    ...
    for i in pbar:
        with torch.no_grad():
            ...
            with network:
                ...
                prompt_retains = [prompt_pair.retain_one,prompt_pair.retain_two,prompt_pair.retain_three,prompt_pair.retain_four,prompt_pair.retain_five]
                ## NDL 
                negative_denoised_latents = []
                retain_denoised_latents = []
                for i,np in enumerate(negative_prompt_embeddings):
                    negative_denoised_latents.append(train_util.diffusion(
                        unet,
                        noise_scheduler,
                        latents,
                        train_util.concat_embeddings(
                ...
                for i in range(len(prompt_retains)):
                    retain_denoised_latents.append(train_util.diffusion(
                        unet,
                        noise_scheduler,
                        latents,
                        train_util.concat_embeddings(
                    ...
                    # print(len(retain_denoised_latents))
                #print("negative denoised latents generated!!!")
            ...
            for np,ndl in zip(negative_prompt_embeddings,negative_denoised_latents):
                negative_latents.append(train_util.predict_noise(
                    unet,
                    noise_scheduler,
                    current_timestep,
                    ndl,
                ...
                negative_latents[-1].requires_grad_ = False

            #print("neg latents generated!!")
        ...
        # ------------------------- Retain Set Regularization part -----------------------------
        if prompt_pair.action == "erase_with_retain":
            ...
            with network:
                retain_latents = [None,None,None,None,None]
                # print(len(retain_denoised_latents))
                for i in range(5):
                    retain_latents[i] = train_util.predict_noise(
                        unet,
                        noise_scheduler,
                        current_timestep,
                        retain_denoised_latents[i],
                        train_util.concat_embeddings(
                    prompt_pair.unconditional,
                    prompt_retains[i],
                    prompt_pair.batch_size,
            ...
            with torch.no_grad():
                retain_latents_ori = [None,None,None,None,None]
                for i in range(5):
                    retain_latents_ori[i] = train_util.predict_noise(
                        unet,
                        noise_scheduler,
                        current_timestep,
                        retain_denoised_latents[i],
                    ...
                    retain_latents_ori[i].requires_grad_ = False 
        ...
        loss = prompt_pair.loss(
            ...
            positive_latents=positive_latents, # positive prompts would be retain won't these ? - retain prompts
            neutral_latents=neutral_latents, # null concept 
            retain_latents = retain_latents, # latents of retain concepts
            retain_latents_ori = retain_latents_ori, # latents of retain concepts from original model
            anchor_latents=anchor_latents, #None
            anchor_latents_ori=anchor_latents_ori, # None
            negative_latents = negative_latents # negative prompts like a photo of many wss, a photo of many <target-class>
        )
...
def main(args):
    ...
    # print("prompts FILEEEEEEEEE",config.prompts_file)
